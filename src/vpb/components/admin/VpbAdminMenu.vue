  ...
  <div @focusout="hideDropdown()">
    <div class=" bg-white shadow-lg flex flex flex-col" v-if="showOptions">
      ...
      <template v-for="post in editablePosts" >
        <a @click="onOptionClick(post)"  v-if="activePostName !== post.name " class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900">
...
export default {
  ...
  setup(props, ctx){
    ...
    return {editablePosts, activePost, showOptions, hideDropdown, toggleDropdown , onOptionClick, adminPath,activePostName}
  }
}
</script>
